...
sshpass -p `echo $PROJECT_PASSWORD | base64 -d` scp .tmp/image.tar "$PROJECT_USERNAME@$PROJECT_VM_IP:~"
sshpass -p `echo $PROJECT_PASSWORD | base64 -d` ssh "$PROJECT_USERNAME@$PROJECT_VM_IP" <<EOF
